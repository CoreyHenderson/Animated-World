...
void main()
{
	...
	// Wing height calculation
	if (VertexST.y > 0.5)
		Position.y += sin((pauto / 20) - (VertexST.y * 2)) * 0.7 * Position.y;
	
	// Feathers and Feet height
	if (VertexST.x > 0.5)
		Position.y += sin((pauto / 20) - (VertexST.y * 2)) * 0.2 * Position.y;

	// Hover height
	Position.y += sin((pauto / 20) + 1) * 0.09;
