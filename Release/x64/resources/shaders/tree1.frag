...
void main()
{
  ...
  /*************************************************************************************/
	...
	vec3 trunkMarks [] = vec3 [] ( lightbrown, lightbrown, lightbrown, brown, lightbrown, lightbrown, lightbrown );	 	

	// Create noise variable for the different colour ranges
	float colourIntensity = 0.89;
	float tfrequency = 7.0;
	
	float pixelPosition = 0.5;
	
	if (s < pixelPosition && t < pixelPosition)
		tfrequency = 3.5;
  ...
  /*************************************************************************************/
	...
	// Leaf marks //
	float markFrequency = 226.0;
	float markVisibility = 0.52;
	float mark = (cnoise (vec2 (markFrequency * s, markFrequency * t))) / markVisibility;
	...
	// Leaf marks //
	markFrequency = 581.0;
	mark = (cnoise (vec2 (markFrequency * s, markFrequency * t))) / markVisibility;
  ...
  /*************************************************************************************/
	...
	vec3 LightColour = vec3 (0.4, 0.8, 0.4);
